...
int main(int argc, char const *argv[])
{
	...
	while (1){
		arg = readLineArguments(args, numberMaxArgs, line, bufferSize);
		if (arg == -1)
			break;
		...
		if (!strcmp(args[0], "run")){
			...
			if (pid == 0){
				printf("SOU FILHO DO PID %d. TENHO PID %d.\n", getppid(), getpid());
				...
				if ((pid % 2) != 0) /* Pid do filho impar */
					printf("NOVO FILHO COM PID IMPAR: %d\n", pid);
